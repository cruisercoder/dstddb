...
void attrGet(T)(void* handle, ub4 handleType, OCIError* error, ub4 attribute, ref T value) {
    ub4 sz = T.sizeof;
    check("OCIAttrGet", OCIAttrGet(handle, OCI_HTYPE_STMT, &value, &sz, attribute,
        error));
}
...
struct Driver(Policy) {
    ...
    static void attrSet(T)(void* handle, ub4 handleType, ub4 attribute, T value) {
        ub4 sz = T.sizeof;
        check("OCIAttrSet", OCIAttrSet(stmt, OCI_HTYPE_STMT, &value, sz, attribute,
            error));
    }
    ...
    struct Conversion {
        ...
        private static const Conversion[4] info = [{SQLT_INT, SQLT_INT}, {
            SQLT_NUM, SQLT_STR}, {SQLT_CHR, SQLT_STR}, {SQLT_DAT, SQLT_DAT}];

            static int get(int colType) {
                // needs improvement
                foreach (ref i; info) {
                    if (i.colType == colType)
                        return i.bindType;
                }
                throw new DatabaseException("unknown conversion for column: " ~ to!string(colType));
            }
        }
        ...
        struct BindInfo {
            ...
            static const BindInfo[4] info = [{SQLT_INT, &charBinder}, {
                SQLT_STR, &charBinder}, {SQLT_STR, &charBinder}, {
                    ...
                    static void bind(ref BindContext ctx) {
                        ...
                        throw new DatabaseException("bind not supported: " ~ to!string(colType));
                    }
                ...
                struct Database {
                    ...
                    this(string defaultURI_) {
                        ...
                        check("OCIEnvCreate", OCIEnvCreate(&env, mode, null,
                            null, null, null, 0, null));

                        check("OCIHandleAlloc", OCIHandleAlloc(env,
                            cast(void**)&error, OCI_HTYPE_ERROR, 0, null));
                    }
                ...
                struct Connection {
                    ...
                    hhkk:
                        ...
                        check("OCILogon: ", OCILogon(db_.env, error, &svc_ctx,
                            ...
                            cast(const OraText*) dbname.ptr, cast(ub4) dbname.length));
                    ...
                    ~this() {
                        if (svc_ctx)
                            check("OCILogoff", OCILogoff(svc_ctx, error));
                    }

                    static string getDBName(Source src) {
                        string dbName = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)" ~ "(HOST=" ~ src.server
                ...
                struct Statement {
                    ...
                    this(Connection* con_, string sql_) {
                        ...
                        check("OCIHandleAlloc", OCIHandleAlloc(con.db.env,
                            cast(void**)&stmt, OCI_HTYPE_STMT, 0, null));
                    ...
                    ~this() {
                        ...
                        if (stmt)
                            check("OCIHandleFree", OCIHandleFree(stmt, OCI_HTYPE_STMT));
                        // stmt = null? needed
                    }
                    ...
                    void prepare() {
                        ...
                        check("OCIStmtPrepare", OCIStmtPrepare(stmt, error,
                            cast(OraText*) sql.ptr, cast(ub4) sql.length,
                    ...
                    void query() {
                        ...
                        check("OCIStmtExecute", OCIStmtExecute(con.svc_ctx,
                            stmt, error, iters, 0, null, null, OCI_COMMIT_ON_SUCCESS));
                    }
             ...
             */
                    ...
                    private T attrGet(T)(ub4 attribute) {
                        return attrGet!T(stmt, error, attribute);
                    }

                    private void attrGet(T)(ub4 attribute, ref T value) {
                        return attrGet(stmt, error, attribute, value);
                    }

                    private void attrSet(T)(ub4 attribute, T value) {
                        return attrSet(stmt, error, attribute, value);
                    }
                    ...
                    static void attrGet(T)(OCIStmt* stmt, OCIError* error,
                        ub4 attribute, ref T value) {
                        return attrGet(stmt, OCI_HTYPE_STMT, error, attribute, value);
                    }

                    static void attrGet(T)(void* handle, ub4 handleType,
                        OCIError* error, ub4 attribute, ref T value) {
                        ub4 sz = T.sizeof;
                        check("OCIAttrGet", OCIAttrGet(handle, OCI_HTYPE_STMT,
                            &value, &sz, attribute, error));
                    }
                ...
                struct Result {
                    ...
                    void build_describe() {
                        ...
                        for (int i = 0; i < columns; ++i) {
                            ...
                            OCIParam* col;
                            check("OCIParamGet", OCIParamGet(stmt.stmt,
                                OCI_HTYPE_STMT, error, cast(void**)&col, i + 1));

                            check("OCIAttrGet", OCIAttrGet(col,
                                OCI_DTYPE_PARAM, &d.ora_name, &d.name_len, OCI_ATTR_NAME,
                                error));

                            check("OCIAttrGet", OCIAttrGet(col,
                                OCI_DTYPE_PARAM, &d.oType, null, OCI_ATTR_DATA_TYPE,
                                error));

                            check("OCIAttrGet", OCIAttrGet(col,
                                OCI_DTYPE_PARAM, &d.size, null, OCI_ATTR_DATA_SIZE,
                                error));

                            d.name = to!string(cast(char*)(d.ora_name)[0 .. d.name_len]);
                            info("describe: name: ", d.name, ", type: ",
                                d.oType, ", size: ", d.size);
                        }
                    ...
                    void build_bind() {
                        ...
                        for (int i = 0; i < columns; ++i) {
                            ...
                            check("OCIDefineByPos", OCIDefineByPos(stmt.stmt,
                                &d.define, error, cast(ub4) i + 1, b.data.ptr,
                                b.allocSize, b.oType, cast(dvoid*) b.ind.ptr, // check
                                cast(ub2*) b.length.ptr, null, OCI_DEFAULT));

                            if (rowArraySize > 1) {
                                check("OCIDefineArrayOfStruct",
                                    OCIDefineArrayOfStruct(d.define, error,
                                    b.allocSize, sb2.sizeof, ub2.sizeof, 0));
                            }
                        }
                    }
                    ...
                    int fetch() {
                        ...
                        status = OCIStmtFetch2(stmt.stmt, error, rowArraySize,
                            OCI_FETCH_NEXT, 0, OCI_DEFAULT);

                        if (rowArraySize > 1) {
                            // clean up
                            ub4 value;
                            check("OCIAttrGet", OCIAttrGet(stmt.stmt,
                                OCI_HTYPE_STMT, &value, null, OCI_ATTR_ROWS_FETCHED,
                                error));
                            rowsFetched = value;
                        }
                        else {
