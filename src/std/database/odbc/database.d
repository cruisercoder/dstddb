...
struct Driver(Policy) {
    ...
    struct Database {
        ...
        this(string defaultURI_) {
            info("Database");
            allocator = Allocator();
            check("SQLAllocHandle", SQLAllocHandle(SQL_HANDLE_ENV, SQL_NULL_HANDLE,
                &env));
        ...
        void showDrivers() {
            ...
            info("DRIVERS:");
            while (SQL_SUCCEEDED(ret = SQLDrivers(env, direction, driver.ptr,
                    driver.sizeof, &driver_ret, attr.ptr, attr.sizeof, &attr_ret))) {
    ...
    struct Connection {
        ...
        this(Database* db_, Source source_) {
            ...
            check("SQLConnect", SQL_HANDLE_DBC, con, SQLConnect(con,
                cast(SQLCHAR*) toStringz(source.server), SQL_NTS,
                cast(SQLCHAR*) toStringz(source.username), SQL_NTS,
                cast(SQLCHAR*) toStringz(source.password), SQL_NTS));
            connected = true;
        }
    ...
    struct Statement {
        ...
        ~this() {
            ...
            if (stmt)
                check("SQLFreeHandle", SQLFreeHandle(SQL_HANDLE_STMT, stmt));
            // stmt = null? needed
        }
        ...
        void inputBind(int n, ref Bind bind) {
            ...
            check("SQLBindParameter", SQLBindParameter(stmt,
                cast(SQLSMALLINT) n, SQL_PARAM_INPUT, b.bindType, b.dbtype, 0,
                0, b.data, b.allocSize, null));
        }
        ...
        void query() {
            if (!binds) {
                ...
                SQLRETURN ret = SQLExecDirect(stmt, cast(SQLCHAR*) toStringz(sql),
                    SQL_NTS);
        ...
        void exec() {
            check("SQLExecDirect", SQLExecDirect(stmt, cast(SQLCHAR*) toStringz(sql),
                SQL_NTS));
        }
    ...
    struct Result {
        ...
        void build_describe() {
            ...
            for (int i = 0; i < columns; ++i) {
                ...
                check("SQLDescribeCol", SQLDescribeCol(stmt.stmt,
                    cast(SQLUSMALLINT)(i + 1), cast(SQLCHAR*) d.name,
                    cast(SQLSMALLINT) Describe.nameSize, &d.nameLen, &d.type,
                    &d.size, &d.digits, &d.nullable));
        ...
        void build_bind() {
            ...
            for (int i = 0; i < columns; ++i) {
                ...
                check("SQLBINDCol", SQLBindCol(stmt.stmt,
                    cast(SQLUSMALLINT)(i + 1), b.bindType, b.data, b.size, &b.len));

                info("output bind: index: ", i, ", type: ", b.bindType,
                    ", size: ", b.size, ", allocSize: ", b.allocSize);
            }
        }
    ...
    static void throw_detail()(SQLHANDLE handle, SQLSMALLINT type, string msg) {
        ...
        do {
            ret = SQLGetDiagRec(type, handle, ++i, cast(char*) state, &native,
                cast(char*) text, text.length, &len);
