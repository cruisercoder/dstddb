...
struct Driver(Policy) {
    ...
    static auto toTypedPtr(T)(void[] data) {
    ...
    static struct DBGen(Driver) {
        ...
        static void[] create(string uri) {
            return construct!Database(alloc, uri);
        }

        static void destroy(void[] db) {
            destruct!Database(alloc, db);
        }
        ...
        static DBVtable vtable = {&create, &destroy, &features,};
    }
    ...
    static struct ConGen(Driver) {
        ...
        static void destroy(void[] con) {
            destruct!Connection(alloc, con);
        }

        static ConVTable vtable = {&create, &destroy,};
    }
    ...
    static struct StmtGen(Driver) {
        ...
        static void destroy(void[] stmt) {
            destruct!Statement(alloc, stmt);
        }

        static void prepare(void[] stmt) {
            toTypedPtr!Statement(stmt).prepare();
        }

        static void query(void[] stmt) {
            toTypedPtr!Statement(stmt).query();
        }
        ...
        static void variadicQuery(void[] stmt, ref BindArgs a) {
            ...
            case 0:
                break;
            case 1:
                constructQuery(s, a[0]);
                break;
            case 2:
                constructQuery(s, a[0], a[1]);
                break;
            case 3:
                constructQuery(s, a[0], a[1], a[2]);
                break;
            default:
                throw new DatabaseException("too many args");
            }

        }

        static StmtVTable vtable = {
            &create, &destroy, &prepare, &query, &variadicQuery,};
        }
        ...
        struct Database {
            ...
            static void register(DB)(string name) {
                ...
                info("poly register: ", "name: ", name, ", " "type: ",
                    typeid(DB), "index: ", drivers.length);
            }
            ...
            const(Feature[]) features() {
                ...
                if (driver)
                    return driver.dbVtable.features(db);
                static const FeatureArray f = [//Feature.InputBinding,
                //Feature.DateBinding,
            ...
            ~this() {
                if (isDatabaseSet) {
                    if (!db.ptr)
                        throw new DatabaseException("db not set");
        ...
        struct Connection {
            ...
            this(Database* database_, Source source_) {
                database = database_;
                source = source_;
                if (!database.isDatabaseSet)
                    throw new DatabaseException("no database set");
