...
alias AsyncDatabase(T) = BasicDatabase!(Async!T,T);
...
struct Async(Policy) {
    alias Allocator = Policy.Allocator;
    alias Database = Impl!Policy.Database;
    alias Connection = Impl!Policy.Connection;
    alias Statement = Impl!Policy.Statement;
    alias Result = Impl!Policy.Result;
    ...
    /*
       ...
       net_async_status status = mysql_real_connect_nonblocking_init(mysql,null);

       check("mysql_real_connect", mysql_real_connect_nonblocking(
       mysql,
       cast(cstring) toStringz(source.server),
       cast(cstring) toStringz(source.username),
       cast(cstring) toStringz(source.password),
